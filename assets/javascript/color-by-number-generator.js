	...
	*/	
	...
	redraw: function(width) {
		...
		if(this.image.src) {
			...
			var maps = this.drawImage(this.image, this.inputContext, this.previewContext, this.previewModeColor);

			this.drawColorSwatches(this.colorSwatchContext, this.uniqueColorsArray, maps[1]);
		}
		if(this.DEBUG) {
			var date2 = new Date();
			var timestamp2 = date2.getMilliseconds();
			console.log("timestamp2 = " + timestamp2);
			const timeDiff = timestamp2 - timestamp;
	...
	drawImage: function(image, inputContext, previewContext, previewModeColor) {
		this.log('drawImage');
	  	...
	  	*/
	  	var maps = this.getImageData(image, inputContext, true);
	  	var initialFrequencyMap = maps[0];
	  	var grid = maps[1];

	  	var initialFrequencyMapSorted = new Map([...initialFrequencyMap.entries()].sort());

	  	var maps2 = this.getMaps(initialFrequencyMapSorted, this.numberColors);
	  	var frequencyMap = maps2[0];
	  	var toleranceMap = maps2[1];
	...
	drawColorSwatches: function(context, colorsArray, toleranceMap) {
		this.log('colorsArray', colorsArray);
		this.log('toleranceMap', toleranceMap);
;
		var colorDiffsMap = new Map();
		for(var i = 0; i < colorsArray.length-1; i++) {
			for(var j = i+1; j < colorsArray.length; j++) {
				// const difference = '0x'+(colorsArray[j] - colorsArray[i]).toString(16);
				var difference = this.subtractHexColor(colorsArray[j], colorsArray[i]);
				colorDiffsMap.set(difference, [colorsArray[j], colorsArray[i]]);


				// console.log('0x'+colorsArray[j] + '- 0x'+colorsArray[i] + ' = ' + difference);
			}
		}
		var colorDiffsMapSorted = new Map([...colorDiffsMap.entries()].sort());
		// colorsArrayDiffs.sort();
		this.log("colorDiffsMapSorted", colorDiffsMapSorted);

		var mapIterator = colorDiffsMapSorted.entries();
		this.log("colorDiffsMap.entries", colorDiffsMapSorted.entries());
		this.log("length", mapIterator.length);

		var k = 0;
		while(k < colorDiffsMapSorted.size) {
			var value = mapIterator.next().value;
			this.log("value: ", value);
			var key = this.getByValue(toleranceMap, value[0]);
			// console.log("key= " + key);
			k++;
		}
	...
	getToleranceMap: function(mapArr, tolerance) {
		...
		while(i < mapArr.length) {
			...
			if(i < mapArr.length - 1) {
				...
				while((i < mapArr.length - 1) && (Math.abs(this.subtractHexColor(color1, color2)) < this.colorTolerance*this.colorToleranceUIRatio)) {
	...
	subtractHexColor: function(c1, c2) {
